...
TEST(ApiServerHelpersTest, SerializationAndDetailBuildersReturnExpectedPayloads) {
    ...
    std::stringstream output;
    ReservationSystem system(input, output);
    Customer* api_customer = system.addCustomerInternal("Builder User", 40, 800.0, false);
    ASSERT_NE(api_customer, nullptr);
    std::string booking_error;
    Booking* api_booking = system.createBookingInternal(api_customer->getPersonId(), "FL101", "4A", booking_error);
    ASSERT_NE(api_booking, nullptr);

    const Airplane* flight = system.findAirplaneByFlightNumber("FL101");
    ASSERT_NE(flight, nullptr);
    json airplane_details = build_airplane_details(system, *flight);
    const auto seat_it = std::find_if(
        airplane_details.at("seats").begin(),
        airplane_details.at("seats").end(),
    ...
    EXPECT_EQ(
        find_confirmed_booking_for_seat(system.getBookingsForTest(), "FL101", "4A"),
        nullptr);

    json customer_details = build_customer_details(system, *api_customer);
...
TEST_F(ApiServerRoutesTest, SupportsAutoGeneratedCustomersAndBookingErrorStatuses) {
    ...
    ASSERT_TRUE(auto_customer_response);
    ASSERT_EQ(auto_customer_response->status, 201);
    const json auto_customer = json::parse(auto_customer_response->body);
...
TEST_F(ApiServerRoutesTest, CancelAndSwapRoutesMapErrorsAndSuccess) {
