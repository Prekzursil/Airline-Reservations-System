...
from tests.quality_script_provider_tests import (
...
def _normalize_lcov_fixture():
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        (repo_root / "src" / "named.py").write_text(
            "print('hello')\n",
            encoding="utf-8",
        )
        (repo_root / "ignored.py").write_text(
            "print('ignore')\n",
            encoding="utf-8",
        )
...
class SecurityHTTPAndHelpersTests(unittest.TestCase):
    def test_request_https_payload_builds_safe_headers_and_handles_json_payloads(self) -> None:
        ...
        self.assertEqual(request_args[1], "/repos/owner/repo")
        self.assertEqual(request_args[2], b'{"ok": true}')
        self.assertEqual(request_args[3]["Accept"], "application/json")
        self.assertEqual(request_args[3]["Content-Type"], "application/json")
