    ...
    def test_quality_secrets_artifacts_exclude_present_secret_metadata(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            try:
                ...
                self.assertIn("missing_secrets", payload)
                self.assertIn("missing_vars", payload)
                ...
                self.assertIn("SNYK_TOKEN", payload["missing_secrets"])
                self.assertIn("SENTRY_AUTH_TOKEN", payload["missing_secrets"])
