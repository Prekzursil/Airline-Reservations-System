...
function BookingCustomerSelector({
  ...
  return (
    <div style={{ marginTop: '10px' }}>
      <label htmlFor="customerSelectBooking" style={{ marginRight: '10px' }}>
        Select Customer:
      </label>
      {bookingSelector}
      <button type="button" onClick={onConfirmBooking} disabled={loadingCustomers}>
...
const SeatMap = ({ seats, flightNumber, onBookingSuccess = null }) => {
  ...
  const handleConfirmBooking = async () => {
    ...
    try {
      ...
      setPendingCancellationId(null);
      setBookingStatus(
        `Booking successful! ID: ${result.bookingId}. Seat: ${result.seatId} for Customer: ${result.customerId}`,
      );
