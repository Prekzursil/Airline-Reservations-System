...
- **CMake + Build Tool:** CMake 3.20+ and a native build tool (`mingw32-make`/`make`, Ninja, or MSBuild).
- **Googletest (for tests):** Fetched automatically by CMake during configure.

### 2.2. C++ API Server (for GUI)

- Same as C++ Backend.
- **cpp-httplib:** Fetched automatically by CMake during configure.
- **nlohmann/json:** Fetched automatically by CMake during configure.
...
- No manual setup of `cpp-httplib`, `nlohmann/json`, or `googletest` is required. CMake fetches them during configuration.

1. **React Frontend Dependencies:**
...
- **Manual Compilation (using Makefile):**
  ...
  mingw32-make test                      # Build and run C++ tests through CTest
  .\build\airline_reservation_system.exe # Run the console app
  ```
...
- Dependencies (`cpp-httplib`, `nlohmann/json`, `googletest`) are fetched by CMake into `build/_deps/` at configure time.
