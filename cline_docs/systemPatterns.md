...
- The `Airplane` class will _contain_ a collection of `Seat` objects (composition, as seats are integral to an airplane).
- The `ReservationSystem` class will _manage_ collections of `Airplane`, `Customer`, and `Booking` objects (aggregation, as these objects can exist independently but are managed by the system).
